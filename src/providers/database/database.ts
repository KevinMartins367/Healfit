  ...
  private insertDefaultItems(db: SQLiteObject) {
    ...
    .then((data: any) => {
      ...
      if (data.rows.item(0).qtd == 0) {
        ...
        // Criando as tabelas
        db.sqlBatch([
          ['insert into clientes (email, password, altura, user_id, pessoa_id, api_token) values (?,?,?,?,?,?)', 
          ['teste@mail.com',sha3_512('12345'), '1.60', '1', '1', sha3_512('teste12345')]]
        ])
    ...
    db.executeSql('select COUNT(id) as qtd from treinos', {})
    .then((data: any) => {
      //Se não existe nenhum registro
      if (data.rows.item(0).qtd == 0) {
        ...
        // Criando as tabelas
        db.sqlBatch([
          ['insert into treinos (type) values (?)', ['A']],
          ['insert into treinos (type) values (?)', ['B']],
          ['insert into treinos (type) values (?)', ['C']],
          ['insert into treinos (type) values (?)', ['D']],
          ['insert into treinos (type) values (?)', ['E']]
        ])
        .then(() => console.log('Dados padrões treinos incluídos '))
        .catch(e => console.error('Erro ao incluir dados padrões 4', e));
      }
    })
    .catch(e => console.error('Erro ao consultar a qtd de treinos', e));

    db.executeSql('select COUNT(id) as qtd from seutreinos', {})
    .then((data: any) => {
      //Se não existe nenhum registro
      if (data.rows.item(0).qtd == 0) {
    ...
    .catch(e => console.error('Erro ao consultar a qtd de seutreinos', e));
    ...
    .then((data: any) => {
      ...
      if (data.rows.item(0).qtd == 0) {
        ...
        db.sqlBatch([
          ...
          ['insert into exercicios (nome, IMG, descricao, periodo, duracao, link, user_id, intensity, treino_id) values (?,?,?,?,?,?,?,?,?)',   
          ['prancha','http://localhost:8000/imagens/','0', '2', '120', '0', '1', '1', '1']],
          ['insert into exercicios (nome, IMG, descricao, periodo, duracao, link, user_id, intensity, treino_id) values (?,?,?,?,?,?,?,?,?)',   
    ...
    .catch(e => console.error('Erro ao consultar a qtd de pesos', e));
